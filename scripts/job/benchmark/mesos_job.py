...
class MesosJob:
  ...
  def mesos_run_helper(self, hostname, port):
    try:
      ret = subprocess.call("./mesos-execute --master=%s:%d --name=%s --command=\"%s %s\" --instances=%d" \
                            % (hostname, port, self.desc.name, self.root_task.desc.binary, " ".join(self.root_task.desc.args), \
