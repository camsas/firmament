...
void KnowledgeBaseSimulator::AddMachineSample(
  ...
  machine_stats.set_timestamp(current_time);
  ...
  for (auto& task_id_rd : task_id_to_rd) {
    TaskStats* task_stat = FindOrNull(task_stats_, task_id_rd.first);
...
void KnowledgeBaseSimulator::EraseStats(TaskID_t task_id) {
  task_stats_.erase(task_id);
}

void KnowledgeBaseSimulator::SetTaskStats(TaskID_t task_id,
