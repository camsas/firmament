...
GoogleTraceBridge::~GoogleTraceBridge() {
  // TODO(ionel): Check what we need to delete.
  for (auto& task_id_td : trace_task_id_to_td_) {
    delete task_id_td.second;
  }
  for (auto& job_id_jd : job_id_to_jd_) {
    delete job_id_jd.second;
  }
  for (auto& machine_id_rd : trace_machine_id_to_td_) {
...
ResourceDescriptor* GoogleTraceBridge::AddMachine(
  ...
  CHECK(InsertIfNotPresent(&trace_machine_id_to_td_, machine_id, rd));
...
void GoogleTraceBridge::RemoveMachine(uint64_t machine_id) {
  trace_machine_id_to_td_.erase(machine_id);
...
void GoogleTraceBridge::TaskCompleted(
  ...
  // task_map_ and task_bindings_, task_id_to_end_time_, res_id_to_task_id_.
