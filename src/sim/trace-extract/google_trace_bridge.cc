...
GoogleTraceBridge::GoogleTraceBridge(
    ...
    topology_manager_(new machine::topology::TopologyManager),
  ...
  scheduler_ = new scheduler::FlowScheduler(
      ...
      object_store_, task_map_, knowledge_base_, topology_manager_,
      messaging_adapter_, this, root_uuid, "http://localhost", params);
}

GoogleTraceBridge::~GoogleTraceBridge() {
  for (auto& job_id_jd : trace_job_id_to_jd_) {
    delete job_id_jd.second;
  }
  for (auto& ti_td : *task_map_) {
    delete ti_td.second;
  }
  // We don't have to delete TaskDescriptors from trace_task_id_to_td_
  // because we delete them when we iterate over task_map_.
  ...
  // 2) machine_res_id_pus_ because rds are deleted when we clean
  // the  resource_map_.
  delete scheduler_;
...
ResourceDescriptor* GoogleTraceBridge::AddMachine(
  ...
  scheduler_->RegisterResource(res_id, false, true);
  CHECK(InsertIfNotPresent(&trace_machine_id_to_rtnd_, machine_id,
                           new_machine));
  DFSTraverseResourceProtobufTreeReturnRTND(
      new_machine, boost::bind(&GoogleTraceBridge::GetMachinePUs,
                               this, _1, res_id));
  return rd_ptr;
}
...
void GoogleTraceBridge::AddTaskStats(
  ...
  knowledge_base_->SetTaskStats(td_ptr->uid(), *task_stats);
...
void GoogleTraceBridge::GetMachinePUs(ResourceTopologyNodeDescriptor* rtnd_ptr,
...
void GoogleTraceBridge::TaskCompleted(
  ...
  knowledge_base_->EraseStats(td_ptr->uid());
}
...
JobDescriptor* GoogleTraceBridge::PopulateJob(uint64_t job_id) {
  ...
  InsertOrUpdate(&trace_job_id_to_jd_, job_id, jd_ptr);
  InsertOrUpdate(&job_id_to_trace_job_id_, new_job_id, job_id);
...
void GoogleTraceBridge::ResetUuidAndAddResource(
  ...
  // TODO(ionel): Check if we need to set the type of the resource.
  string new_uuid;
