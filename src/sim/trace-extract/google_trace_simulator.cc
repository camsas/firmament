...
void GoogleTraceSimulator::LogSolverRunStats(
    const boost::timer::cpu_timer timer, uint64_t run_solver_by,
  ...
  if (stats_file_) {
    ...
    if (FLAGS_batch_step == 0) {
      // online mode
      double scheduling_latency = run_solver_by;
      ...
      fprintf(stats_file_, "%jd,%lf,%lf,%lf,%lf,", run_solver_by,
              scheduling_latency, algorithm_time, flow_solver_time,
              total_runtime);
    } else {
      // batch mode
      fprintf(stats_file_, "%jd,%lf%lf%lf,", run_solver_by,
...
void GoogleTraceSimulator::LogStartOfSolverRun(uint64_t run_solver_by) {
  ...
  LOG(INFO) << "Scheduler run for time: " << run_solver_by;
...
uint64_t GoogleTraceSimulator::NextRunSolverBy(uint64_t cur_run_solver_by,
  ...
  if (FLAGS_batch_step == 0) {
    ...
    cur_run_solver_by += static_cast<uint64_t>(time_to_solve);
    ...
    if (FLAGS_incremental_flow) {
      ...
      events_.insert(make_pair(cur_run_solver_by, event));
    }
  } else {
    // we're in batch mode
    cur_run_solver_by += FLAGS_batch_step;
  }
  return cur_run_solver_by;
}
    ...
    while (!feof(f_task_events_ptr)) {
      if (fscanf(f_task_events_ptr, "%[^\n]%*[\n]", &line[0]) > 0) {
        ...
        if (vals.size() != 13) {
          ...
          num_events++;
          if (SimulationCompletionCondition(task_time, num_events,
                                            num_scheduling_rounds)) {
            return;
          }

          while (task_time > run_solver_by) {
            ProcessSimulatorEvents(run_solver_by, machine_tmpl);
            ...
            if (!flow_graph_->graph_changes().empty()) {
              ...
              run_solver_by = RunSolver(run_solver_by);
            ...
            run_solver_by = max(next_event, run_solver_by);
            VLOG(1) << "Run solver by " << run_solver_by;
...
uint64_t GoogleTraceSimulator::RunSolver(uint64_t run_solver_by) {
  double algorithm_time;
  double flow_solver_time;
  LogStartOfSolverRun(run_solver_by);
  ...
  UpdateFlowGraph(run_solver_by, task_runtime_, task_mappings);
  delete task_mappings;
  // Also update any resource statistics, if required
  UpdateResourceStats();

  // Log stats to CSV file
  LogSolverRunStats(timer, run_solver_by, algorithm_time,
                    flow_solver_time, change_stats);

  return NextRunSolverBy(run_solver_by, algorithm_time);
}
...
bool GoogleTraceSimulator::SimulationCompletionCondition(
    uint64_t task_time,
    uint64_t num_events,
