...
vector<JobDescriptor*>& GoogleTraceExtractor::LoadInitialJobs(
    int64_t max_jobs) {
...
void GoogleTraceExtractor::Run() {
  ...
  vector<JobDescriptor*>& initial_jobs = LoadInitialJobs(FLAGS_num_jobs);
  //LoadInitalTasks();
  ...
  // Add initial jobs
  for (vector<JobDescriptor*>::const_iterator iter = initial_jobs.begin();
