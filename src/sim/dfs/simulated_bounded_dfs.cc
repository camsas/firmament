...
ResourceID_t SimulatedBoundedDFS::PlaceBlockOnMachinesPool(
  ...
  do {
    ...
    if (num_machines_selected > machines->size()) {
      // It's time to refresh the pool because we've likely ran out space.
...
void SimulatedBoundedDFS::GetRandomMachinePool(
  ...
  while (machines->size() < num_machines) {
    if (num_machines_selected > MAX_SAMPLE_POOL * num_machines) {
