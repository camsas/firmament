...
DEFINE_uint64(simulated_quincy_block_size, 512, "The size of a DFS block in MB");
DEFINE_uint64(simulated_dfs_blocks_per_machine, 12288,
              "Number of 512 MB blocks each machine stores. "
              "Defaults to 12288, i.e. 6 TB.");
...
uint64_t SimulatedDataLayerManager::AddFilesForTask(
  ...
  if (!long_running_service) {
    ...
    uint64_t num_blocks = input_size / FLAGS_simulated_quincy_block_size / MB_TO_BYTES;
    if ((input_size / MB_TO_BYTES) % FLAGS_simulated_quincy_block_size != 0) {
      num_blocks++;
    }
