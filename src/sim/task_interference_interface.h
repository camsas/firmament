...
#ifndef FIRMAMENT_SIM_TASK_INTERFERENCE_INTERFACE_H
#define FIRMAMENT_SIM_TASK_INTERFERENCE_INTERFACE_H
...
class TaskInterferenceInterface {
 public:
  ...
  virtual void OnTaskCompletion(uint64_t current_time_us,
                                TaskDescriptor* td_ptr,
                                ResourceID_t res_id,
                                vector<TaskEndRuntimes>* tasks_end_time) = 0;
  virtual void OnTaskEviction(uint64_t current_time_us,
                              TaskDescriptor* td_ptr,
                              ResourceID_t res_id,
                              vector<TaskEndRuntimes>* tasks_end_time) = 0;
  virtual void OnTaskMigration(uint64_t current_time_us,
                               TaskDescriptor* td_ptr,
                               ResourceID_t old_res_id,
                               ResourceID_t res_id,
                               vector<TaskEndRuntimes>* tasks_end_time) = 0;
  virtual void OnTaskPlacement(uint64_t current_time_us,
                               TaskDescriptor* td_ptr,
                               ResourceID_t res_id,
                               vector<TaskEndRuntimes>* tasks_end_time) = 0;
};
...
#endif  // FIRMAMENT_SIM_TASK_INTERFERENCE_INTERFACE_H
