...
DEFINE_double(quincy_interference_wait_time_factor, 0.5,
              "The Quincy wait time factor");
DEFINE_double(quincy_interference_preferred_machine_data_fraction, 0.1,
              "Threshold of proportion of data stored on machine for it to be "
              "on preferred list.");
DEFINE_double(quincy_interference_preferred_rack_data_fraction, 0.1,
              "Threshold of proportion of data stored on rack for it to be on "
              "preferred list.");
DEFINE_int64(quincy_interference_tor_transfer_cost, 1,
             "Cost per unit of data transferred in core switch.");
// Cost was 2 for most experiments, 20 for constrained network experiments
DEFINE_int64(quincy_interference_core_transfer_cost, 2,
             "Cost per unit of data transferred in core switch.");
DEFINE_bool(quincy_interference_update_costs_upon_machine_change, true,
            "True if the costs should be updated if a machine is added or "
            "removed");
DEFINE_int64(quincy_interference_positive_cost_offset, 2592000, "Value to offset "
...
DEFINE_bool(quincy_interference_no_scheduling_delay, false, "Offset cost to "
            ...
            "possible");
DEFINE_double(quincy_interference_colocation_factor, 0.1, "Colocation factor"
            " which increases cost as a function of already running tasks.");
...
ArcDescriptor QuincyInterferenceCostModel::TaskToUnscheduledAgg(
  ...
  if (FLAGS_quincy_interference_no_scheduling_delay) {
    ...
    no_delay_offset = 39 * FLAGS_quincy_interference_core_transfer_cost;
  }
  if (td.priority() == 1000) {
    ...
    return ArcDescriptor(
        100 + FLAGS_quincy_interference_positive_cost_offset + no_delay_offset,
        1ULL, 0ULL);
  }
  ...
  return ArcDescriptor(
      static_cast<Cost_t>(total_unscheduled_time *
                          FLAGS_quincy_interference_wait_time_factor /
                          static_cast<int64_t>(MICROSECONDS_IN_SECOND) +
                          FLAGS_quincy_interference_positive_cost_offset + no_delay_offset),
      1ULL, 0ULL);
}
...
double QuincyInterferenceCostModel::InterferenceCostFactor(
  ResourceID_t resource_id) {
    ResourceStatus* rs = FindPtrOrNull(*resource_map_, resource_id);
    uint64_t num_running_tasks = rs->topology_node().resource_desc().
    num_running_tasks_below();
    // Simple linear model
    return 1.0 + FLAGS_quincy_interference_colocation_factor * num_running_tasks;
  }
...
ArcDescriptor QuincyInterferenceCostModel::TaskToResourceNode(
    ...
    ResourceID_t resource_id) {
      ...
      double colocation_factor = InterferenceCostFactor(resource_id);

      if (machines_data) {
        Cost_t* transfer_cost = FindOrNull(*machines_data, resource_id);
        if (transfer_cost) {
          uint64_t adjusted_cost = static_cast<uint64_t>(std::round(
            *transfer_cost * colocation_factor));

          return ArcDescriptor(static_cast<Cost_t>(adjusted_cost
            + FLAGS_quincy_interference_positive_cost_offset), 1ULL, 0ULL);
        } else {
          ...
          return ArcDescriptor(static_cast<Cost_t>(adjusted_cost
            + FLAGS_quincy_interference_positive_cost_offset),
              1ULL, 0ULL);
        }
      } else {
...
Cost_t QuincyInterferenceCostModel::GetTransferCostToNotPreferredRes(
    TaskID_t task_id,
    ResourceID_t res_id) {
...
// NOTE Adjusting time cost-discount for colocation:
double colocation_factor = InterferenceCostFactor(machine_res_id);
Cost_t adjusted_discount = static_cast<Cost_t>(std::round(task_executed_for
                             * colocation_factor));
...
// already offsetted.
return ArcDescriptor(arc_cost_cap.cost_ - adjusted_discount, 1ULL, 0ULL);
}
...
ArcDescriptor QuincyInterferenceCostModel::TaskToEquivClassAggregator(
  ...
  // NOTE Updating task costs to account for running tasks.
  TaskDescriptor* td_ptr = GetMutableTask(task_id);
  UpdateTaskCosts(td_ptr, ec, false);

  return ArcDescriptor(*transfer_cost + FLAGS_quincy_interference_positive_cost_offset,
                       1ULL, 0ULL);
}
...
void QuincyInterferenceCostModel::AddMachine(
  ...
  if (FLAGS_quincy_interference_update_costs_upon_machine_change) {
...
void QuincyInterferenceCostModel::RemovePreferencesToMachine(ResourceID_t res_id) {
...
void QuincyInterferenceCostModel::RemovePreferencesToRack(EquivClass_t ec) {
...
uint64_t QuincyInterferenceCostModel::ComputeClusterDataStatistics(
...
uint64_t QuincyInterferenceCostModel::ComputeDataStatsForMachine(
...
Cost_t QuincyInterferenceCostModel::ComputeTransferCostToMachine(uint64_t remote_data,
                                                     uint64_t data_on_rack) {
  CHECK_GE(remote_data, data_on_rack);
  return (FLAGS_quincy_interference_tor_transfer_cost * static_cast<int64_t>(data_on_rack) +
          FLAGS_quincy_interference_core_transfer_cost *
          static_cast<int64_t>(remote_data - data_on_rack)) /
    static_cast<int64_t>(BYTES_TO_GB);
}

Cost_t QuincyInterferenceCostModel::ComputeTransferCostToRack(
    EquivClass_t ec,
    uint64_t input_size,
    uint64_t data_on_rack,
...
void QuincyInterferenceCostModel::ConstructTaskPreferredSet(TaskID_t task_id) {
  TaskDescriptor* td_ptr = GetMutableTask(task_id);
  ...
  for (auto& machine_data : data_on_machines) {
    ...
    if (data_on_machine >=
        input_size * FLAGS_quincy_interference_preferred_machine_data_fraction) {
  ...
  if (data_on_ecs.size() < data_layer_manager_->GetNumRacks()) {
    ...
    worst_cluster_cost = FLAGS_quincy_interference_core_transfer_cost *
  ...
  for (auto& rack_data : data_on_ecs) {
    ...
    if (rack_data.second >=
        input_size * FLAGS_quincy_interference_preferred_rack_data_fraction) {
...
void QuincyInterferenceCostModel::UpdateMachineBlocks(
    const DataLocation& location,
    unordered_map<ResourceID_t, unordered_map<uint64_t, uint64_t>,
      boost::hash<ResourceID_t>>* data_on_machines) {
  CHECK_NOTNULL(data_on_machines);
  auto machine_blocks = FindOrNull(*data_on_machines, location.machine_res_id_);
  if (machine_blocks) {
...
void QuincyInterferenceCostModel::UpdateRackBlocks(
    const DataLocation& location,
    unordered_map<EquivClass_t,
...
void QuincyInterferenceCostModel::UpdateTaskCosts(TaskDescriptor* td_ptr,
...
Cost_t QuincyInterferenceCostModel::UpdateTaskCostForRack(TaskDescriptor* td_ptr,
                                              EquivClass_t rack_ec) {
  unordered_map<ResourceID_t, unordered_map<uint64_t, uint64_t>,
  ...
  // Compute how much task data we have on the rack.UpdateTaskCostForRack We only account each
  ...
  for (const auto& machine_res_id : machines_in_rack) {
    ...
    if (machine_blocks) {
      ...
      // NOTE Adjusting by num_running_tasks.
      Cost_t adjusted_cost = static_cast<Cost_t>(std::round(colocation_factor *
                             transfer_cost));
      worst_rack_cost = max(worst_rack_cost, adjusted_cost);
    } else {
      // No blocks on the machine.
      Cost_t transfer_cost = ComputeTransferCostToMachine(input_size, data_on_rack);
      worst_rack_cost = static_cast<Cost_t>(std::round(colocation_factor *
                             transfer_cost));
    }
  }
...
unordered_map<ResourceID_t, Cost_t, boost::hash<ResourceID_t>>*
  QuincyInterferenceCostModel::UpdateTaskPreferredMachineList(
    TaskID_t task_id,
    uint64_t input_size,
    ResourceID_t machine_res_id,
...
void QuincyInterferenceCostModel::UpdateTaskPreferredRacksList(
