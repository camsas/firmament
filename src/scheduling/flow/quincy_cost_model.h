...
class QuincyCostModel : public CostModelInterface {
 public:
  ...
  virtual Cost_t TaskContinuationCost(TaskID_t task_id);
  virtual Cost_t TaskPreemptionCost(TaskID_t task_id);
  // Costs to equivalence class aggregators
  virtual Cost_t TaskToEquivClassAggregator(TaskID_t task_id, EquivClass_t tec);
  ...
  virtual vector<EquivClass_t>* GetTaskEquivClasses(TaskID_t task_id);
  vector<ResourceID_t>* GetOutgoingEquivClassPrefArcs(EquivClass_t tec);
  virtual vector<ResourceID_t>* GetTaskPreferenceArcs(TaskID_t task_id);
  vector<EquivClass_t>* GetEquivClassToEquivClassesArcs(EquivClass_t tec);
  virtual void AddMachine(ResourceTopologyNodeDescriptor* rtnd_ptr);
  virtual void AddTask(TaskID_t task_id);
  virtual void RemoveMachine(ResourceID_t res_id);
  virtual void RemoveTask(TaskID_t task_id);
