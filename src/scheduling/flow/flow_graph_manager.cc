...
DEFINE_bool(
    affinity_selector, false,
    "True for soft affinity contraint, false for hard affinity constraint");
...
FlowGraphManager::FlowGraphManager(
    ...
    TimeInterface* time_manager, TraceGenerator* trace_generator,
  ...
  // Add sink node.
  sink_node_ = graph_change_manager_->AddNode(FlowNodeType::SINK, 0,
                                              ADD_SINK_NODE, "SINK");
...
FlowGraphNode* FlowGraphManager::AddEquivClassNode(EquivClass_t ec) {
  FlowGraphNode* ec_node =
      graph_change_manager_->AddNode(FlowNodeType::EQUIVALENCE_CLASS, 0,
                                     ADD_EQUIV_CLASS_NODE, "AddEquivClassNode");
  ec_node->ec_id_ = ec;
...
void FlowGraphManager::AddResourceTopologyDFS(
  ...
  if (rtnd_ptr->parent_id().empty()) {
    ...
    graph_change_manager_->AddArc(
        ...
        ADD_ARC_BETWEEN_RES, "AddResourceTopologyDFS");
  }
}
...
void FlowGraphManager::AddResourceTopology(
  ...
  if (!rtnd_ptr->parent_id().empty()) {
    ...
    FlowGraphNode* cur_node =
        NodeForResourceID(ResourceIDFromString(rtnd_ptr->parent_id()));
    int64_t running_tasks_delta = static_cast<int64_t>(
...
FlowGraphNode* FlowGraphManager::AddResourceNode(ResourceDescriptor* rd_ptr) {
  ...
  FlowGraphNode* res_node = graph_change_manager_->AddNode(
      FlowGraphNode::TransformToResourceNodeType(*rd_ptr), 0, ADD_RESOURCE_NODE,
      comment.c_str());
  ResourceID_t res_id = ResourceIDFromString(rd_ptr->uuid());
...
FlowGraphNode* FlowGraphManager::AddTaskNode(JobID_t job_id,
  ...
  FlowGraphNode* task_node = graph_change_manager_->AddNode(
      FlowNodeType::UNSCHEDULED_TASK, 1, ADD_TASK_NODE, "AddTaskNode");
...
void FlowGraphManager::ComputeTopologyStatistics(
    FlowGraphNode* node, boost::function<void(FlowGraphNode*)> prepare,
...
void FlowGraphManager::SchedulingDeltasForPreemptedTasks(
    ...
    shared_ptr<ResourceMap_t> resource_map, vector<SchedulingDelta*>* deltas) {
...
void FlowGraphManager::NodeBindingToSchedulingDeltas(
  ...
  if (bound_res) {
    ...
    if (*bound_res != ResourceIDFromString(res.uuid())) {
      ...
      VLOG(2) << "MIGRATION: take " << task.uid() << " off " << *bound_res
              << " and move it to " << res.uuid();
...
void FlowGraphManager::PurgeUnconnectedEquivClassNodes() {
  ...
  for (unordered_map<EquivClass_t, FlowGraphNode*>::iterator it =
           tec_to_node_map_.begin();
...
void FlowGraphManager::RemoveInvalidECPrefArcs(
    const FlowGraphNode& node, const vector<EquivClass_t>& pref_ecs,
...
void FlowGraphManager::RemoveInvalidPrefResArcs(
    const FlowGraphNode& node, const vector<ResourceID_t>& pref_resources,
    DIMACSChangeType change_type) {
  unordered_set<ResourceID_t, boost::hash<boost::uuids::uuid>> res_preferences(
      pref_resources.begin(), pref_resources.end());
  unordered_set<FlowGraphArc*> to_delete;
  for (auto& dst_arc : node.outgoing_arc_map_) {
...
void FlowGraphManager::TaskMigrated(TaskID_t task_id, ResourceID_t old_res_id,
...
void FlowGraphManager::UpdateChildrenTasks(
    TaskDescriptor* td_ptr, queue<TDOrNodeWrapper*>* node_queue,
    unordered_set<uint64_t>* marked_nodes) {
  ...
  for (RepeatedPtrField<TaskDescriptor>::pointer_iterator task_iter =
           td_ptr->mutable_spawned()->pointer_begin();
       task_iter != td_ptr->mutable_spawned()->pointer_end(); ++task_iter) {
...
void FlowGraphManager::UpdateEquivClassNode(
    FlowGraphNode* ec_node, queue<TDOrNodeWrapper*>* node_queue,
    unordered_set<uint64_t>* marked_nodes) {
  CHECK_NOTNULL(ec_node);
  CHECK_NOTNULL(node_queue);
  CHECK_NOTNULL(marked_nodes);
...
void FlowGraphManager::UpdateEquivToEquivArcs(
  ...
  CHECK_NOTNULL(node_queue);
  CHECK_NOTNULL(marked_nodes);
  // TODO(shivramsrivastava): Need to make this independent of cpu mem cost
  // model as of
  // now its soft affinity implementation for cpu mem cost model.
  // Task descriptor should have info on soft or hard affinity constriant, we
  // need to fill
  // this in Poseidon side. Using gflag FLAGS_affinity_selector to decide soft
  // or hard
  // constraint. For hard constraint, GetEquivClassToEquivClassesArcs() gets
  // called.
  // And GetEquivClassToEquivClassesArcsSoftAffinity() gets called for soft
  // constraint.
  if (!FLAGS_affinity_selector) {
    ...
    if (pref_ec) {
      ...
      RemoveInvalidECPrefArcs(*ec_node, no_pref_ec,
                              DEL_ARC_BETWEEN_EQUIV_CLASS);
    }
  } else {
    MachineECs_t* machine_ec_vectors =
        cost_model_->GetEquivClassToEquivClassesArcsSoftAffinity(
            ec_node->ec_id_);
    vector<EquivClass_t>* pref_ec = machine_ec_vectors->pref_ecs;
    if (pref_ec->size() > 0) {
      LOG(INFO)
          << "DEBUG_CPU_SOFT_AFFINITY: Cost model returned PREFERRED resources"
          << pref_ec->size();
    }
    ...
    if (not_pref_ecs->size() > 0) {
      LOG(INFO) << "DEBUG_CPU_SOFT_AFFINITY: Cost model returned nonpreferred "
                   "resources "
                << not_pref_ecs->size();
    }
    if (not_pref_ecs) {
      for (auto& not_pref_ecs_id : *not_pref_ecs) {
        ...
        ArcDescriptor arc_descriptor = cost_model_->EquivClassToEquivClass(
            ec_node->ec_id_, not_pref_ecs_id);
        arc_descriptor.cost_ += preferred_max_cost;
        FlowGraphArc* not_pref_ecs_arc =
            graph_change_manager_->mutable_flow_graph()->GetArc(
                ec_node, not_pref_ecs_node);
        if (!not_pref_ecs_arc) {
          graph_change_manager_->AddArc(
          ...
          graph_change_manager_->ChangeArc(
              not_pref_ecs_arc, arc_descriptor.min_flow_,
        ...
        if (marked_nodes->find(not_pref_ecs_node->id_) == marked_nodes->end()) {
          ...
          node_queue->push(new TDOrNodeWrapper(not_pref_ecs_node,
                                               not_pref_ecs_node->td_ptr_));
        }
      }
    } else {
      vector<EquivClass_t> no_not_pref_ecs;
      RemoveInvalidECPrefArcs(*ec_node, no_not_pref_ecs,
                              DEL_ARC_BETWEEN_EQUIV_CLASS);
    }
    if (pref_ec && !not_pref_ecs) {
      RemoveInvalidECPrefArcs(*ec_node, *pref_ec, DEL_ARC_BETWEEN_EQUIV_CLASS);
      delete pref_ec;
    } else if (!pref_ec && not_pref_ecs) {
      RemoveInvalidECPrefArcs(*ec_node, *not_pref_ecs,
                              DEL_ARC_BETWEEN_EQUIV_CLASS);
      delete not_pref_ecs;
    } else if (not_pref_ecs && pref_ec) {
      vector<EquivClass_t>* all_machine_ecs = new vector<EquivClass_t>();
      all_machine_ecs->reserve(pref_ec->size() + not_pref_ecs->size());
      all_machine_ecs->insert(all_machine_ecs->begin(), pref_ec->begin(),
                              pref_ec->end());
      all_machine_ecs->insert(all_machine_ecs->end(), not_pref_ecs->begin(),
                              not_pref_ecs->end());
      RemoveInvalidECPrefArcs(*ec_node, *all_machine_ecs,
                              DEL_ARC_BETWEEN_EQUIV_CLASS);
      delete pref_ec;
      delete not_pref_ecs;
    }
    ...
    for (auto& pref_res_id : *pref_res) {
      ...
      if (!pref_res_arc) {
        graph_change_manager_->AddArc(
            ...
            arc_descriptor.capacity_, arc_descriptor.cost_, OTHER,
            ADD_ARC_EQUIV_CLASS_TO_RES, "UpdateEquivToResArcs");
...
void FlowGraphManager::UpdateFlowGraph(queue<TDOrNodeWrapper*>* node_queue,
                                       unordered_set<uint64_t>* marked_nodes) {
  CHECK_NOTNULL(node_queue);
  CHECK_NOTNULL(marked_nodes);
...
void FlowGraphManager::UpdateResourceNode(
    FlowGraphNode* res_node, queue<TDOrNodeWrapper*>* node_queue,
    unordered_set<uint64_t>* marked_nodes) {
  CHECK_NOTNULL(res_node);
...
void FlowGraphManager::UpdateResourceStatsUpToRoot(
  ...
  while (true) {
    ...
    if (!parent_node) {
      ...
      uint64_t new_capacity = static_cast<uint64_t>(
          static_cast<int64_t>(parent_arc->cap_upper_bound_) + cap_delta);
      graph_change_manager_->ChangeArcCapacity(parent_arc, new_capacity,
                                               CHG_ARC_BETWEEN_RES,
                                               "UpdateCapacityUpToRoot");
      uint64_t new_num_slots = static_cast<uint64_t>(
          static_cast<int64_t>(parent_node->rd_ptr_->num_slots_below()) +
          slots_delta);
      parent_node->rd_ptr_->set_num_slots_below(new_num_slots);
      uint64_t new_num_running_tasks = static_cast<uint64_t>(
          static_cast<int64_t>(
...
void FlowGraphManager::UpdateResOutgoingArcs(
  ...
  for (unordered_map<uint64_t, FlowGraphArc*>::iterator it =
    ...
    ++it;
    if (!arc->dst_node_->resource_id_.is_nil()) {
      ArcDescriptor arc_descriptor = cost_model_->ResourceNodeToResourceNode(
          *res_node->rd_ptr_, *arc->dst_node_->rd_ptr_);
      graph_change_manager_->ChangeArc(
          arc, arc_descriptor.min_flow_, arc_descriptor.capacity_,
          arc_descriptor.cost_, CHG_ARC_BETWEEN_RES, "UpdateResOutgoingArcs");
...
void FlowGraphManager::UpdateResToSinkArc(FlowGraphNode* res_node) {
  if (res_node->type_ == FlowNodeType::PU) {
    ...
    FlowGraphArc* res_arc_sink =
        graph_change_manager_->mutable_flow_graph()->GetArc(res_node,
                                                            sink_node_);
    ArcDescriptor arc_descriptor =
...
void FlowGraphManager::UpdateRunningTaskNode(
    FlowGraphNode* task_node, bool update_preferences,
    queue<TDOrNodeWrapper*>* node_queue,
    unordered_set<uint64_t>* marked_nodes) {
  CHECK_NOTNULL(task_node);
  FlowGraphArc* running_arc =
  ...
  ArcDescriptor arc_descriptor =
      cost_model_->TaskContinuation(task_node->td_ptr_->uid());
  graph_change_manager_->ChangeArc(running_arc, arc_descriptor.min_flow_,
                                   arc_descriptor.capacity_,
...
void FlowGraphManager::UpdateRunningTaskToUnscheduledAggArc(
  ...
  graph_change_manager_->ChangeArc(unsched_arc, arc_descriptor.min_flow_,
                                   arc_descriptor.capacity_,
...
void FlowGraphManager::UpdateTaskToEquivArcs(
    FlowGraphNode* task_node, queue<TDOrNodeWrapper*>* node_queue,
    unordered_set<uint64_t>* marked_nodes) {
  CHECK_NOTNULL(task_node);
  CHECK_NOTNULL(node_queue);
  CHECK_NOTNULL(marked_nodes);
  ...
  if (pref_ec) {
    for (auto& pref_ec_id : *pref_ec) {
      ...
      ArcDescriptor arc_descriptor = cost_model_->TaskToEquivClassAggregator(
          task_node->td_ptr_->uid(), pref_ec_id);
      FlowGraphArc* pref_ec_arc =
          graph_change_manager_->mutable_flow_graph()->GetArc(task_node,
                                                              pref_ec_node);
      if (!pref_ec_arc) {
...
void FlowGraphManager::UpdateTaskToResArcs(
  ...
  if (pref_res) {
    for (auto& pref_res_id : *pref_res) {
      ...
      // prefer a resource before it is added to the graph.
      CHECK_NOTNULL(pref_res_node);
      ArcDescriptor arc_descriptor = cost_model_->TaskToResourceNode(
          task_node->td_ptr_->uid(), pref_res_id);
      FlowGraphArc* pref_res_arc =
          graph_change_manager_->mutable_flow_graph()->GetArc(task_node,
                                                              pref_res_node);
      if (!pref_res_arc) {
...
FlowGraphNode* FlowGraphManager::UpdateTaskToUnscheduledAggArc(
  ...
  if (!to_unsched_arc) {
    ...
    graph_change_manager_->ChangeArc(to_unsched_arc, arc_descriptor.min_flow_,
                                     arc_descriptor.capacity_,
                                     arc_descriptor.cost_, CHG_ARC_TO_UNSCHED,
...
void FlowGraphManager::UpdateUnscheduledAggNode(FlowGraphNode* unsched_agg_node,
                                                int64_t cap_delta) {
  CHECK_NOTNULL(unsched_agg_node);
  ...
  if (!unsched_agg_sink_arc) {
    ...
    graph_change_manager_->ChangeArc(
        unsched_agg_sink_arc, arc_descriptor.min_flow_, new_capacity,
        arc_descriptor.cost_, CHG_ARC_FROM_UNSCHED, "UpdateUnscheduledAggNode");
  }
}
