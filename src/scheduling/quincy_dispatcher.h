...
class QuincyDispatcher {
 ...
 public:
  QuincyDispatcher(shared_ptr<FlowGraph> flow_graph, bool initial_solver_run)
    : flow_graph_(flow_graph), initial_solver_run_(initial_solver_run), debug_seq_num_(0) {
  }

  map<uint64_t, uint64_t>* Run();

 private:
  uint64_t AssignNode(vector< map< uint64_t, uint64_t > >* extracted_flow, uint64_t node);
  map<uint64_t, uint64_t>* GetMappings(vector< map< uint64_t, uint64_t > >* extracted_flow,
                                         unordered_set<uint64_t> leaves, uint64_t sink);
  vector< map< uint64_t, uint64_t> >* ReadFlowGraph(int fd, uint64_t num_vertices);
