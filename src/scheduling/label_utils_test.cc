...
class LabelUtilsTest : public ::testing::Test {
 protected:
  ...
  void CreateResourceWithLabels(ResourceTopologyNodeDescriptor* rtnd_ptr,
                                string machine_name, const char* key,
                                const char* value) {
  ...
  TaskDescriptor* CreateTaskWithLabels(JobDescriptor* jd_ptr,
                                       uint64_t job_id_seed, const char* key,
                                       const char* value) {
...
TEST_F(LabelUtilsTest, SatisfiesLabelSelector) {
  ...
  CHECK_STREQ(rtnd.mutable_resource_desc()->mutable_labels(0)->key().c_str(),
              "1");
  CHECK_STREQ(rtnd.mutable_resource_desc()->mutable_labels(0)->value().c_str(),
              "One");
  ...
  bool ret = SatisfiesLabelSelector(*(rtnd.mutable_resource_desc()),
                                    td_ptr->label_selectors(0));
  CHECK_EQ(ret, true);
  ...
  ret = SatisfiesLabelSelector(*(rtnd.mutable_resource_desc()),
                               td_ptr->label_selectors(1));
  CHECK_EQ(ret, false);
...
TEST_F(LabelUtilsTest, SatisfiesLabelSelectors) {
  ...
  bool ret = SatisfiesLabelSelectors(*(rtnd.mutable_resource_desc()),
...
TEST_F(LabelUtilsTest, SatisfiesLabelSelector_selectortype_unspecified) {
  ...
  CHECK_STREQ(td_ptr->mutable_label_selectors(0)->key().c_str(), "3");
  CHECK_STREQ(td_ptr->mutable_label_selectors(0)->values(0).c_str(), "Three");
  bool ret = SatisfiesLabelSelector(*(rtnd.mutable_resource_desc()),
                                    td_ptr->label_selectors(0));
  CHECK_EQ(ret, true);
  // label selector 1.
  ...
  CHECK_STREQ(td_ptr->mutable_label_selectors(1)->key().c_str(), "4");
  CHECK_STREQ(td_ptr->mutable_label_selectors(1)->values(0).c_str(), "Four");
  ret = SatisfiesLabelSelector(*(rtnd.mutable_resource_desc()),
                               td_ptr->label_selectors(1));
  CHECK_EQ(ret, false);
}
