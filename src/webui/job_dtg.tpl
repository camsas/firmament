...
function add_node(dat, parent)
{
      ...
      if ( Member == 0)
      {
        ...
        if(parent)
        {
          ...
          console.log("If this appears more than few times, something is " +
                      "wrong, pressumably force can't finish before the next " +
                      "step() call");
        }
      }
      dat.shape = 0;
      ...
      if (dat.spawned.length > 0 ) {
        ...
        for(var i=0; i < dat.spawned.length; i++)
          add_node(dat.spawned[i], tmp);
      }
}
