...
/* Currently using LRU*/
        void Cache::make_space_in_cache() {
            ...
            vector<DataObjectID_t> iterator it = cache->object_list->begin();
            bool cleared = false;
        ...
        void Cache::create_cache(const char* cache_name) {
           ...
           boost::thread t(boost::bind(&Cache::handle_notification_references,
                              reference_not_t));
        ...
        void Cache::handle_notification_references(ReferenceNotification_t* ref) { 
            ...
            while(true) { 
                WriteLock_t lock(ref->mutex);
                if (ref->writable)ref->cond_added.wait(lock, ref->writable); 
                ...
                switch(rd->type()) { 
                        case (ReferenceDescriptor::CONCRETE): 
                            ...
                            rd->AddLocation(store->get_listening_interface()); 
                            break; 
                        case (ReferenceDescriptor::FUTURE): 
                            /* Was future. Make Concrete now*/
                            VLOG(3) << "Reference was a future. Making concrete "  ; 
                            ConcreteReference* conc_ref = new ConcreteReference(id, ref->size, 
                                    new set<string>()); 
                            conc_ref->AddLocation(store->get_listening_interface()); 
                            rd = &(conc_ref->desc()) ; 
                            break ;
                        default:  VLOG(1) << "Unimplemented " ; 
                }    
                ref->writable = true ; 
                lock.unlock() ; 
